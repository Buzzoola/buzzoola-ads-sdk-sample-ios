...
struct RewardedView: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject, ObservableObject, RewardedAdLoaderProtocol, RewardedAdDelegate {

        // MARK: Private properties

        var viewController: UIViewController?

        var adLoader: RewardedAdLoader?

        private var rewarded: RewardedAd?

        var parent: RewardedView

        var showAdClosure: (() -> Void)?
        
        var loadAdClosure: (() -> Void)?

        init(_ parent: RewardedView) {
            self.parent = parent
        }

        func showAd() {
            rewarded?.show(from: viewController ?? UIViewController())
        }
        ...
        // MARK: RewardedAdLoaderProtocol

        func onAdLoaded(ad: any BuzzoolaAdsSDK.RewardedAd) {
            parent.state = .ready
        ...
        func onReward(reward: BuzzoolaAdsSDK.Reward) {
            print("onReward", reward)
        }
    }
